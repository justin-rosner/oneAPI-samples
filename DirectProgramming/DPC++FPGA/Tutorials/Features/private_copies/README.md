...
| What you will learn               | The basic usage of the `private_copies` attribute <br> How the `private_copies` attribute affects loop throughput and resource use <br> How to apply the `private_copies` attribute to variables in your program <br> How to identify the correct `private_copies` factor for your program
...
The `private_copies` attribute enables you to control the number of copies of any memory that this is applied to. This works best when applying to any memory that has single iteration scope, thus enabling simultaneously executed loop iterations.

#### Example: 

Kernels in this tutorial design apply `[[intelfpga::private_copies(N)]]` to two arrays within the outer loop that are used by subsequent inner loops. These inner loops perform some simple math operations before storing the results. The following is an example of a loop nest:

```
for (size_t i = 0; i < kMaxIter; i++) {
  [[intel::private_copies(2)]] int a[kSize];
  [[intel::private_copies(2)]] int b[kSize];
  int c[kSize];
  for (size_t j = 0; j < kSize; j++) {
    a[j] = accessor_array[(i * 4 + j) % kSize] * shift;
  }
  for (size_t j = 0; j < kSize; j++)
    b[j] = SimpleMath(a[j], a[kSize - j - 1]);
  for (size_t j = 0; j < kSize; j++)
    c[j] = SimpleMath(b[j], b[kSize - j - 1]);
  for (size_t j = 0; j < kSize; j++)
    r += c[j];
}  
```

In this example, you only need to have two private copies of arrays a and b in order to have 2 concurrent outer loop iterations. The `private_copies` attribute in this example forces the compiler to create two private copies of the arrays `a` and `b`. Passing the parameter `N` to the `private_copies` attribute limits the number of privately-declared arrays that can be made in that loop. 
...
* How to apply the `private_copies` attribute to variables in your program
...
```
PASSED_fpga_compile
Num private_copies 0 kernel time : 2043.12 ms
Throughput for kernel with private_copies 0: 0.401 GFlops
Num private_copies 1 kernel time : 4163.122 ms
Throughput for kernel with private_copies 1: 0.197 GFlops
Num private_copies 2 kernel time : 2081.313 ms
Throughput for kernel with private_copies 2: 0.394 GFlops
Num private_copies 3 kernel time : 2043.122 ms
Throughput for kernel with private_copies 3: 0.401 GFlops
Num private_copies 4 kernel time : 2043.123 ms
Throughput for kernel with private_copies 4: 0.401 GFlops
Num private_copies 5 kernel time : 2043.122 ms
Throughput for kernel with private_copies 5: 0.401 GFlops
