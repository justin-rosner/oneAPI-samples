...
// Helper function to compute some simple math operations
int SimpleMath(int input_data1, int input_data2) {
  int temp_result = int(sqrt(input_data1)) + int(log(input_data2));
  return int(cos(temp_result)) + int(sin(input_data2)) + int(sqrt(input_data1));
}
    ...
    event e = q.submit([&](handler &h) {
      ...
      h.single_task<Kernel<NumCopies>>([=]() [[intel::kernel_args_restrict]] {
        ...
        for (size_t i = 0; i < kMaxIter; i++) {
          ...
          [[intel::private_copies(NumCopies)]] int b[kSize];
          int c[kSize];
...
int main() {
  ...
  SimpleMathWithShift<5>(selector, a, shift, R5);
  ...
  if (gr != R5[0]) {
    std::cout << "private_copies 5: mismatch: " << R5[0] << " != " << gr
              << " (kernel != expected)" << '\n';
    success = false;
  }
